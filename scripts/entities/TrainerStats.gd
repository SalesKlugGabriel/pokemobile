...
## Pontos extras concedidos fora do nível (recompensa de quest/task) — somam
## na hora de calcular quanto dá pra gastar, mas não vêm do nível do Treinador.
...
## + pontos de bônus (recompensa de quest/task, fora da progressão de nível).
func get_available_points() -> int:
	return max(0, (trainer_level - 1) + _bonus_points - _spent_total)
...
func deserialize(data: Dictionary) -> void:
	...
	_bonus_points = data.get("bonus_points", 0)
