	...
	if world_scene:
		...
		if warp_zones:
			for w in warp_zones.get_children():
				...
				and not w.target_map.contains("RockTunnel") and not w.target_map.contains("SafariZone") and not w.target_map.contains("RocketHideout") and not w.target_map.contains("VictoryRoad"):
					alvos_indevidos += 1
